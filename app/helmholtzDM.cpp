...
//This simpler application illustrates how NPME with Laplace kernel and
...
int main (int argc, char *argv[])
{
  ...
  size_t nCharge;
  vector<double> coord;
  vector<_Complex double> charge;
  if (!NPME_ReadSingleBoxChargeCoordComplex (coordFile.c_str(),
  ...
  keyword.AddKeyword ("funcType",   "Helmholtz");
  ...
  NPME_InterfaceComplex_Helmholtz_DM npme;
  if (!npme.SetUp (keyword, nCharge, &coord[0], printLog, ofs_log))
  {
  ...
  vector<_Complex double> Vpme(nCharge*4);
