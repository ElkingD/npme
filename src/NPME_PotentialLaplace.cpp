...
#include "NPME_PotentialLaplace.h"
  ...
  const size_t blockSize)
{
  ...
  size_t k;
  #pragma omp parallel shared(V1, Q1, coord, nBlock, remain) private(k) default(none) num_threads(nProc)
  {
  ...
  const size_t blockSize)
{
  ...
  size_t k;
  #pragma omp parallel shared(V, charge, coord, cluster) private(k) default(none) num_threads(nProc)
  {
    #pragma omp for schedule(dynamic)
    for (k = 0; k < nCluster; k++)
    {
