...
//output: X [N1*N2*N3/8]  = compact even/odd contribution to theta
{
  ...
  long int k;
  #pragma omp parallel for schedule(dynamic) shared(X, F, lamda1_2, lamda2_2, lamda3_2, N1, N2, N3, M1, M2, M3, a1, a2, a3) private(k) num_threads(nProc) default(none)
  for (k = 0; k < M1*M2; k++)
  {
    long int n1, n2;
...
//output: X [N1*N2*N3/8]  = compact even/odd contribution to theta
{
  ...
  long int k;
  #pragma omp parallel for schedule(dynamic) shared(T, X, C1, C2, C3, N1, N2, N3, M1, M2, M3, a1, a2, a3) private(k) num_threads(nProc) default(none)
  for (k = 0; k < M1*M2; k++)
  {
    long int n1, n2;
    NPME_ind2D_2_n1_n2 (k, M2, n1, n2);
...
//output: X [N1*N2*N3/8]  = compact even/odd contribution to theta
{
  ...
  long int k;
  #pragma omp parallel for schedule(dynamic) shared(X, F, lamda1_2, lamda2_2, lamda3_2, N1, N2, N3, M1, M2, M3, a1, a2, a3, reverseSignX, reverseSignY, reverseSignZ) private(k) num_threads(nProc) default(none)
  for (k = 0; k < M1*M2; k++)
  {
    long int n1, n2;
    NPME_ind2D_2_n1_n2 (k, M2, n1, n2);
