...
//note that x1, y1, z1 are modified
{
  ...
  long int k;
  #pragma omp parallel for shared (f, x1, x2, y1, y2, z1, z2, Tx1, Tx2, Ty1, Ty2, Tz1, Tz2, func, N1, N2, N3) private(k) default(none) num_threads(nProc)
  for (k = 0; k < N1*N2; k++)
  {
    long int i1, i2;
...
//1) z-component
  ...
  #pragma omp parallel for schedule(static) shared (f, M1, M2, M3,  N3) private(n) default(none) num_threads(nProc)
  for (n = 0; n < M1*M2; n++)
  {
...
//2) y-component
  ...
  mkl_set_num_threads (1);
  #pragma omp parallel for schedule(static) shared (f, M1, M2, M3, N2) private(n) default(none) num_threads(nProc)
  for (n = 0; n < M1*M3; n++)
  {
...
//3) x-component
  ...
  mkl_set_num_threads (1);
  #pragma omp parallel for schedule(static) shared (fTmp, M1, M2, M3, N1) private(n) default(none) num_threads(nProc)
  for (n = 0; n < M2*M3; n++)
  {
...
//-Z <= z <= Z
{
  double *x1  = (double *) NPME_malloc (N1*sizeof(double), 64);
  double *y1  = (double *) NPME_malloc (N2*sizeof(double), 64);
  double *z1  = (double *) NPME_malloc (N3*sizeof(double), 64);
...
//output f[nRandom]
{
  ...
  long int n;
  #pragma omp parallel for schedule(static) shared (f, Fk, Xexp, Yexp, Zexp, N1, N2, N3, nRandom) private(n, fTmp) default(none) num_threads(nProc)
  for (n = 0; n < N1*N2; n++)
  {
    fTmp.resize(nRandom);
...
//output f[nRandom]
{
  ...
  std::vector<_Complex double> fTmp;
  long int n;
  #pragma omp parallel for schedule(static) shared (f, Fk, Xexp, Yexp, Zexp, M1, M2, M3, nRandom) private(n, fTmp) default(none) num_threads(nProc)
  for (n = 0; n < M1*M2; n++)
  {
    fTmp.resize(nRandom);
